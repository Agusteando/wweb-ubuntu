...
export default class SplitPdfCommand {
  public type = "Command";
  public instructions =
    "!split <1,3-5> (Responda a un PDF) - Divide el PDF en los rangos especificados.";
  ...
  async handle(message: Message, client: Client, _session: UserSession) {
    ...
    if (!clientId || !clientSecret) {
      await message.reply(
        "Error del sistema: Las credenciales de Adobe PDF no están configuradas."
      );
      return;
    }
    ...
    if (media.mimetype !== "application/pdf") {
      await message.reply(
        "Formato de archivo no soportado. Por favor, cite un archivo PDF."
      );
      return;
    }
    ...
    await new Promise<void>((resolve) => {
      tmp.dir(
        { unsafeCleanup: true },
        async (err, dirPath, cleanupCallback) => {
          if (err) {
            await message.reply(
              "Ocurrió un error al preparar el entorno de procesamiento."
            );
            return resolve();
          }
          ...
          try {
            ...
            const splitOperation =
              PDFServicesSdk.SplitPDF.Operation.createNew();
            ...
            if (ranges.length === 0 || !ranges[0]) {
              await message.reply(
                "Por favor, especifique un rango de páginas válido. Ejemplo: !split 1,3-5"
              );
              return resolve();
            }
            ...
            for (let i = 0; i < result.length; i++) {
              ...
              filesPromises.push(
                result[i].saveAsFile(outputPath).then(() => outputPath)
              );
            }
            ...
            console.info(
              `[SplitPdfCommand] PDF processing completed. Sending ${
                outputPaths.length
              } output file(s) to ${destination}${
                sourceDestination && sourceDestination !== destination
            ...
            for (let index = 0; index < outputPaths.length; index += 1) {
              ...
              const filename = `split_${index + 1}_of_${
                outputPaths.length
              }.pdf`;
              const splitPdfMessage = await MessageMedia.fromFilePath(
                outputPath
              );
