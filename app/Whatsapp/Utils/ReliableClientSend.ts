...
async function snapshotOutgoingIds(
  client: Client,
  chatId: string
): Promise<string[]> {
  ...
  try {
    return await page.evaluate(
      async ({ chatId: targetChatId }: { chatId: string }) => {
        ...
        const buildId = (id: any): string | undefined => {
          ...
          return `${Boolean(id?.fromMe)}_${remote}_${stanza}${
      ...
      async ({ targetChatId, knownIds, since, expected }: any) => {
        ...
        for (const model of candidates) {
          ...
          const modelBody = String(
            model?.body ?? model?.caption ?? mediaData?.caption ?? ""
          );
          const modelCaption = String(
            model?.caption ?? mediaData?.caption ?? model?.body ?? ""
          );
          const modelMime = String(
            model?.mimetype ?? mediaData?.mimetype ?? ""
          );
          const modelFilename = String(
            model?.filename ?? mediaData?.filename ?? ""
          );
...
async function verifyOutboundMessage(
  ...
  for (let attempt = 0; attempt < 24; attempt += 1) {
    const messageData = await findNewOutboundMessage(
      client,
      chatId,
      beforeIds,
      startedAt,
...
function instantiateMessage(
  client: Client,
  messageData: BrowserMessageData
): any {
...
export function installReliableClientSend(
  client: Client,
  clientId = "unknown"
): void {
  ...
  (client as any).sendMessage = async (
    ...
    const operation = previous
      ...
      .then(async () => {
        ...
        const verifiedData = await verifyOutboundMessage(
          client,
          chatId,
          beforeIds,
          startedAt,
        ...
        if (verifiedData) {
          ...
          console.warn(
            `[outbound:${clientId}] WhatsApp returned no message object for ${chatId}; recovered delivery confirmation from chat history${
