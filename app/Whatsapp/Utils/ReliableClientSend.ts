...
type BrowserMessageData = Record<string, any>
...
function buildSignature(content: SendContent, options: SendOptions): OutboundSignature {
...
async function snapshotOutgoingIds(client: Client, chatId: string): Promise<string[]> {
  const page = (client as any).pupPage
  if (!page?.evaluate) return []

  try {
    return await page.evaluate(async ({ chatId: targetChatId }: { chatId: string }) => {
      ...
      const buildId = (id: any): string | undefined => {
        ...
        if (!remote || !stanza) return undefined
      ...
      return chat.msgs
        .getModelsArray()
        .filter((model: any) => Boolean(model?.id?.fromMe ?? model?.fromMe))
        .slice(-200)
        .map((model: any) => buildId(model?.id))
        .filter(Boolean)
    }, { chatId })
  } catch (_error) {
    return []
...
async function findNewOutboundMessage(
  client: Client,
  chatId: string,
  beforeIds: string[],
  startedAt: number,
  signature: OutboundSignature
): Promise<BrowserMessageData | null> {
  const page = (client as any).pupPage
  if (!page?.evaluate) return null

  try {
    return await page.evaluate(
      async ({ targetChatId, knownIds, since, expected }: any) => {
        const w = globalThis as any
        const serialized = (value: any): string | undefined => {
          if (!value) return undefined
          if (typeof value === 'string') return value
        ...
        const toModelData = (model: any) => {
          try {
            if (w.WWebJS?.getMessageModel) return normalize(w.WWebJS.getMessageModel(model))
          } catch (_error) {}
          try {
        ...
        for (const model of candidates) {
          ...
          let score = Number(model?.t ?? model?.timestamp ?? 0) / 1_000_000_000

          if (expected.kind === 'text') {
            if (modelBody === String(expected.body ?? '')) score += 100
            else if (expected.body && modelBody.includes(String(expected.body))) score += 50
            else continue
          } else if (expected.kind === 'media') {
            if (!hasMedia) continue
            score += 20
            if (expected.filename && modelFilename === expected.filename) score += 100
            else if (expected.filename && modelFilename) score -= 10
            if (expected.mimetype && modelMime === expected.mimetype) score += 60
            if (expected.caption && modelCaption === expected.caption) score += 50
          } else {
            score += 10
          }
...
async function verifyOutboundMessage(
  client: Client,
  chatId: string,
  beforeIds: string[],
  startedAt: number,
  ...
  for (let attempt = 0; attempt < 24; attempt += 1) {
    ...
    if (messageData && getSentMessageId(messageData)) return messageData
    await delay(250)
  }
  return null
}

function instantiateMessage(client: Client, messageData: BrowserMessageData): any {
  const whatsapp = require('whatsapp-web.js')
  const MessageCtor = whatsapp.Message
  if (typeof MessageCtor !== 'function') return messageData
...
export function installReliableClientSend(client: Client, clientId = 'unknown'): void {
  ...
  ;(client as any).sendMessage = (chatId: string, content: SendContent, options: SendOptions = {}) => {
    ...
    const operation = previous
      ...
      .then(async () => {
        ...
        if (verifiedData) {
          const recoveredId = getSentMessageId(verifiedData)
          console.warn(
            `[outbound:${clientId}] WhatsApp returned no message object for ${chatId}; recovered delivery confirmation from chat history${recoveredId ? ` (${recoveredId})` : ''}.`
          )
        ...
        throw new Error(
          `WhatsApp did not create a verifiable outbound message for ${chatId}. The send operation returned no message and no new outgoing message appeared in the chat.`
        )
      })
