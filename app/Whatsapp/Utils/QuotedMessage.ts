...
type BrowserResolution = {
  status: 'ok' | 'not_found' | 'no_media' | 'media_unavailable'
  messageData?: UnknownRecord
  media?: {
    data: string
    mimetype: string
...
type ResolverMode = 'current' | 'quoted'

function asRecord(value: unknown): UnknownRecord | null {
  return value && typeof value === 'object' ? (value as UnknownRecord) : null
}

function asSerialized(value: unknown): string | undefined {
...
function addSerialized(target: string[], value: unknown) {
  const text = asSerialized(value) ?? asString(value)
  if (text && text.includes('_') && text.includes('@') && !target.includes(text)) target.push(text)
}

function collectKnownReferences(raw: UnknownRecord, message: Message) {
  const serializedIds: string[] = []
  const stanzaIds: string[] = []
  const remoteIds: string[] = []
  const participantIds: string[] = []

  const quoted = asRecord(raw.quotedMsg) ?? asRecord(raw.quotedMessage)
  const context =
    asRecord(raw.contextInfo) ??
  ...
  const messageId = asRecord((message as any).id)
  for (const value of [
    quotedId?.remote,
    quoted?.remote,
    quoted?.remoteJid,
...
export function buildQuotedMessageIdCandidates(message: Message): string[] {
  const raw = asRecord((message as any)._data) ?? {}
  ...
  for (const stanzaId of references.stanzaIds) {
    if (stanzaId.includes('_') && stanzaId.includes('@')) {
      if (!candidates.includes(stanzaId)) candidates.push(stanzaId)
      continue
    }
    ...
    for (const remoteId of references.remoteIds) {
      for (const fromMe of [false, true]) {
        ...
        if (!candidates.includes(directId)) candidates.push(directId)
...
function buildCurrentMessageIdCandidates(message: Message): string[] {
  const candidates: string[] = []
  const id = asRecord((message as any).id)
...
async function resolveInBrowser(message: Message, mode: ResolverMode, includeMedia: boolean): Promise<BrowserResolution> {
  const client = (message as any).client
  const page = client?.pupPage

  if (!page?.evaluate) return { status: 'not_found' }
  ...
  try {
    return await page.evaluate(async (resolverInput: any) => {
      ...
      const serialized = (value: any): string | undefined => {
        if (!value) return undefined
        if (typeof value === 'string') return value
        return value._serialized ?? value.$1
      }
      const uniquePush = (target: string[], value: any) => {
      ...
      const normalize = (value: any, depth = 0): any => {
        ...
        if (Array.isArray(value)) {
          for (const item of value) normalize(item, depth + 1)
          return value
        }
        if (value.$1 !== undefined && value._serialized === undefined) value._serialized = value.$1
        for (const key of Object.keys(value)) normalize(value[key], depth + 1)
        return value
      }
      const collections = safeRequire('WAWebCollections')
      const messages = collections?.Msg
      if (!messages) return { status: 'not_found' }
      ...
      const modelData = (model: any) => {
        ...
        try {
          ...
          if (typeof data.id?.remote === 'object') data.id.remote = serialized(data.id.remote)
      ...
      const matchesKnownReference = (model: any, stanzaIds: string[], remoteIds: string[], participantIds: string[]) => {
        const id = model?.id
        const stanza = serialized(id?.id) ?? id?.id
        if (!stanza || !stanzaIds.includes(String(stanza))) return false

        const remote = serialized(id?.remote) ?? id?.remote
        if (remoteIds.length && remote && !remoteIds.includes(String(remote))) return false

        const participant = serialized(id?.participant) ?? id?.participant
        if (participantIds.length && participant && !participantIds.includes(String(participant))) return false

        return true
      }

      let currentModel: any = null
      for (const id of resolverInput.currentIds as string[]) {
      ...
      if (resolverInput.mode === 'quoted') {
        for (const id of resolverInput.quotedIds as string[]) {
        ...
        const stanzaIds: string[] = [...(resolverInput.stanzaIds ?? [])]
        const remoteIds: string[] = [...(resolverInput.remoteIds ?? [])]
        const participantIds: string[] = [...(resolverInput.participantIds ?? [])]
      ...
      if (!targetModel) return { status: 'not_found' }
      ...
      const hasMedia = Boolean(
        targetModel.directPath ||
          targetModel.mediaData ||
          targetModel.mediaObject ||
          messageData?.directPath,
      )
      if (!hasMedia) return { status: 'no_media', messageData }
      ...
      try {
        ...
        // WhatsApp Web 2.3000.1043xxx may throw a minified `r` here.
        // Continue with cache and direct decryption paths below.
      }
      ...
      const toBase64 = async (input: any): Promise<string | null> => {
      ...
      if (!data) return { status: 'media_unavailable', messageData }
...
export async function getQuotedMessageSafely(message: Message, _context: string): Promise<Message | null> {
  if (!message.hasQuotedMsg) return null

  const resolution = await resolveInBrowser(message, 'quoted', false)
  if (resolution.status !== 'ok' || !resolution.messageData) return null

  const client = (message as any).client
  return new ((message as any).constructor)(client, resolution.messageData) as Message
}

export async function downloadQuotedMediaSafely(
  message: Message,
  _context: string,
  ...
  if (resolution.status !== 'ok' || !resolution.media) return null

  return new MessageMedia(
    resolution.media.mimetype,
    resolution.media.data,
    resolution.media.filename,
    resolution.media.filesize,
  )
}

export async function downloadMessageMediaSafely(
  message: Message,
  _context: string,
): Promise<MessageMedia | null> {
